...
module.exports.reload = async (client, channel, language) => {
    ...
    const messageData = client.textChannel[textChannelType].get("getMessage").musicPannel.message(language);
    messageData.components = await client.textChannel[textChannelType].get("getMessage").musicPannel.actionRow(language, {
        client: client
    });
    ...
    if (messagesColl.size !== 1 || messages[0][1].author.id !== client.user.id) {
        ...
        Channel.send(channel, messageData, (msg) => {
            client.dataBase.get("message").insert(client, {
                ...
                "v_type_mess": textChannelType,
                "v_name_mess": "musicPannel",
                "v_emoji": [],
                "v_type": [],
                "v_data": {}
            }, (error) => {
