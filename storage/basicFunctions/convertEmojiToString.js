...
module.exports.emojiListToStringList = (emojiList) => {
...
module.exports.stringListToEmojiList = (emojiList) => {
    for (let emojiIndex = 0; emojiIndex < emojiList.length; emojiIndex++) {
        const theEmoji = emojiList[emojiIndex];
        if (!testIfIsEmojis(theEmoji)) emojiList[emojiIndex] = convertEmoji[theEmoji];
        if (!emojiList[emojiIndex]) console.log("Emoji " + emojiList[emojiIndex] + " is unknown");
    }
